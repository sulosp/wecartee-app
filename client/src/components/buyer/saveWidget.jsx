...
export default function SaveWidget() {
    ...
    const uniqueCategories = [
        ...new Set(
            BUSINESS_DIRECTORY_LIST.map((business) => business.category).flat(),
        ),
    ];

    const filteredBusinesses =
        selectedCategory === "All"
            ? BUSINESS_DIRECTORY_LIST.filter(
                  (savedBusiness) => savedBusiness.save,
              )
            : BUSINESS_DIRECTORY_LIST.filter(
                  (savedBusiness) =>
                      savedBusiness.save &&
                      savedBusiness.category.includes(selectedCategory),
              );
        ...
        <div className="flex flex-col justify-start items-start h-auto bg-white gap-5 p-20 rounded-lg">
            ...
            <div className="flex flex-row gap-5 flex-wrap ">
                {filteredBusinesses.map((savedBusiness) => (
                    <img
                        key={savedBusiness.key}
                        src={savedBusiness.iconPath}
                        alt={savedBusiness.business}
                        className="w-15 h-15 rounded-full"
                    />
                ))}
                <Link
                    to="savedBusinesses"
                    className="w-15 h-15 bg-surface-100 rounded-full flex flex-col justify-center items-center border-dashed border-primary-100 border-2"
                >
